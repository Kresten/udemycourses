...
SELECT name, cd.facilities.facid, SUM(slots) AS total_slots_booked
FROM cd.facilities
...
-- But since sub queries generally offer slightly worse performance:
SELECT starttime
FROM cd.bookings AS bks
JOIN cd.members AS mbs ON mbs.memid = bks.memid
WHERE firstname || ' ' || surname = 'David Farrell'; 
